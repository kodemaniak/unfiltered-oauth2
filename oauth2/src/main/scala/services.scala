...
trait ServiceResponses {
  ...
  def invalidRedirectUri[T](request: Req[T], uri: Option[String], client: Option[Client]): ResponseFunction[Any]



  /** @return a function that provides a user notification that a request was made with an invalid client */
  def invalidClient[T](request: Req[T]): ResponseFunction[Any]
}
